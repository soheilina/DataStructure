...
void BinaryTree :: addNode( shared_ptr<Node>& parent, const shared_ptr<Node>& node ){ // Inserts a new node asccending form
    ...
    if( parent == nullptr ){
        ...
        // traverse to left
        addNode( parent->getLeftNode(), node); // a temp variable can be used too: shared_ptr<Node>& n{ parent->getLeftNode() };
    }
    else if( node->getData() > parent->getData() ){ // if bigger
        // traverse to right
        addNode( parent->getRightNode(), node); // a temp variable can be used too: shared_ptr<Node>& n{ parent->getRightNode() };
    }else { // if equal: the node already exists. Do nothing.
        cout << "The node already exists." << endl;
    }
}

void BinaryTree :: printTreePreOrder(const shared_ptr<Node> node){ // print first the root node, then left, then right (up-to-down)
    if(node == nullptr){
...
void BinaryTree :: printTreeInOrder(const shared_ptr<Node> node, int nodeDepth){ // print left subtree, then root, then right subtree ( ACENDING )
    if(node == nullptr){
        // Base case: Do nothing
    }
    else{
...
void BinaryTree :: printTreePostOrder(const shared_ptr<Node> node ){ // print left subtree, then right subtree, then root (down-to-top)
    if(node == nullptr){
        // Base case: Do nothing
    }
    else{
...
void BinaryTree :: printTreeReverseOrder(const shared_ptr<Node> node, int nodeDepth ){ // print right subtree, then root, then left subtree ( DECENDING )
    if(node == nullptr){
        // Base case
    }
    else{
...
shared_ptr<Node>& BinaryTree :: findNode( shared_ptr<Node>& startNode, int nodeValue )const{ // NOTE: It has O(log2). An inefficient solution would be search all branches for equality regardless of bigger or smaller
...
void BinaryTree :: removeNode( shared_ptr<Node>& startNode ){
    if(startNode->getLeftNode() == nullptr && startNode->getRightNode() == nullptr){ // has no child
        // Base case: remove it.
        startNode = nullptr;
    }
    else if(startNode->getLeftNode() != nullptr && startNode->getRightNode() == nullptr){ // has only left children
        // substitute the node by its left subtree
        startNode = startNode->getLeftNode();
    }
    else if(startNode->getLeftNode() == nullptr && startNode->getRightNode() != nullptr){ // has only right children:
        ...
        shared_ptr<Node>& n = getMin(startNode->getRightNode()); // returns the min child which is in right subtree
...
shared_ptr<Node>& BinaryTree :: getMin( shared_ptr<Node>& startNode ) const {
    if(startNode == nullptr){
        ...
        //Base case:
        return startNode; // NOTE: this returns the result of very inner function (last left node) to its own 'one' upper function
    }
    else {
        // traverse to left
        return getMin( startNode->getLeftNode() ); // NOTE: Each inner function returns the result to its upper function untill it gets to the 1st one and then out.
    }
}
